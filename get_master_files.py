...
def concat_fars_ext(f): 

    '''Creates csv file of combined spreadsheets from the FARS download folder'''
    
    years = [2018, 2019, 2020, 2021, 2022] # Range of years that we want the data for
    filepath_in = input("Folder Location: ") 
    os.chdir(f"{filepath_in}")
    if "csv" not in f: # Takes the file we want from FARS as sole argument, catches user forgetting file extension
        filename_in = f"{f}.csv"
    else:
        filename_in = f
    ...
    for year in years:
        ...
        if i:
            ...
            master_file_df['Year'] = year # Create year column
            i = False
